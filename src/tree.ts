...
export function buildTree(treeArr: any[], config: TreeConfig) {
	...
	function asciiTree(arr: any[], level: number = 0) {
		...
		for (let i = 0; i < arr.length; i++) {
			const item = arr[i];
			const isLast =
				i === arr.length - 1 || (typeof item === 'object' && arr.length > 1);
...
export async function fetchTree(url: string) {
	...
	if (branch === undefined) {
		try {
			...
			try {
				sha = await getTreeSha(owner as string, repo as string, 'master');
			} catch (err) {
				throw new FormError(
		...
		try {
			sha = await getTreeSha(owner as string, repo as string, branch as string);
		} catch (err) {
